...
class Randomness(ABC):
    ...
    def reset(self):
        ...
        self.state = torch.zeros((l,self.vocab_size)).long().to(self.device)
...
class ExternalRandomness(Randomness):
    ...
    def __init__(self, secret_key, device, vocab_size, key_len=512, random_size = None):
        ...
        self.xi = torch.tensor([[r.random() for _ in range(self.key_len*self.random_size)] for r in self.rng], dtype=torch.float32).reshape(len(self.rng), self.key_len, self.random_size).to(self.device)


    def reset(self):
        super().reset()
        l = 1 if type(self.secret_key) != list else len(self.secret_key)
        self.shift = torch.randint(self.key_len, (l,self.vocab_size)).to(self.device)
