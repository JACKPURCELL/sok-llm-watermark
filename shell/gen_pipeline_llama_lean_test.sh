...
echo "start $watermark_type"
    ...
    echo "start $model"
        ...
        CUDA_VISIBLE_DEVICES="$gpus" nohup python watermark_reliability_release/evaluation_pipeline.py --overwrite_output_file True --watermark "$watermark_type" --wandb_project llama_test_lean --run_name eva-c4-"$watermark_type"-"$model"  --input_dir ~/sok-llm-watermark/runs/token_200/"$watermark_type"/c4/"$model"test &
