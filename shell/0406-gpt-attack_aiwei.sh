#!/bin/bash
#===============================================================================
# usecommand nohup bash shell/0406-gpt-attack_aiwei.sh > shell/0406-gpt-attack_aiwei.log 2>&1 &
#===============================================================================
    ...
    for model in "${models[@]}"; do
        for token in  "${tokens[@]}"; do
          for attack_type in "${attack_types[@]}"; do
             # 对每个元素执行python命令
             CUDA_VISIBLE_DEVICES="$gpus" python watermark_reliability_release/attack_pipeline.py --wandb False --overwrite_args True  --wandb False --input_dir /home/jkl6486/sok-llm-watermark/runs_server3/"$token"/"$watermark_type"/c4/"$model"/gptattack_3  --output_dir /home/jkl6486/sok-llm-watermark/runs_server3/"$token"/"$watermark_type"/c4/"$model"/gptattack_4 --attack_method "$attack_type" --overwrite_output_file True
          done
