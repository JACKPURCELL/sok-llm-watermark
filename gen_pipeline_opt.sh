...
echo "start $watermark_type"
    ...
    echo "start $model"
        ...
        echo "start $temp"
            ...
            CUDA_VISIBLE_DEVICES="$gpus" nohup python watermark_reliability_release/evaluation_pipeline.py --overwrite_output_file True --watermark "$watermark_type" --wandb_project gen-c4-v2.0-200tokens-"$temp"temp --run_name eva-c4-"$watermark_type"-"$model"  --input_dir "/home/ljc/sok-llm-watermark/runs/token_200/$watermark_type"/c4/"$model"-"$temp"  --overwrite_args True  --overwrite_output_file True   --lower_tolerance_T 50 --upper_tolerance_T 100 &
        done
    done
    echo "finish $model"
done
