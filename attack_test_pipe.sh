...
for model in "${models[@]}"; do
    ...
    for attack_type in "${attack_types[@]}"; do
        ...
        echo python watermark_reliability_release/attack_pipeline.py --run_name "$attack_type"-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/"$attack_type" --attack_method "$attack_type"
    done


    #synonym
    for synonym_prob in "${synonym_probs[@]}"; do
        echo python watermark_reliability_release/attack_pipeline.py --run_name synonym-"$synonym_prob"-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/synonym-"$synonym_prob" --attack_method synonym --synonym_p "$synonym_prob"
    done
    ...
    for dipper_lex in "${dipper_lexs[@]}"; do
        echo python watermark_reliability_release/attack_pipeline.py --run_name dipper-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/dipper_l"$dipper_lex"_o0 --attack_method dipper --lex "$dipper_lex"
    done
    echo python watermark_reliability_release/attack_pipeline.py --run_name dipper-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/dipper_l60_o20 --attack_method dipper --lex 60 --order 20
