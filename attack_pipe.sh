...
for watermark_type in "${watermark_types[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        #     CUDA_VISIBLE_DEVICES=3 python watermark_reliability_release/attack_pipeline.py --run_name dipper-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/dipper_l"$dipper_lex"_o0 --attack_method dipper --lex "$dipper_lex"
        # done
        # CUDA_VISIBLE_DEVICES=3 python watermark_reliability_release/attack_pipeline.py --run_name dipper-attack-"$watermark_type"-c4 --input_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"  --output_dir ~/sok-llm-watermark/runs/"$watermark_type"/c4/"$model"/dipper_l60_o20 --attack_method dipper --lex 60 --order 20
        echo "finish $model"
    done
