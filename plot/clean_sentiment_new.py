...
for i, watermark_type in enumerate(watermark_types):
    ...
    wm_avg_var_dict_list.append(avg_var_dict)
    
fig, axs = plt.subplots(3, 5, figsize=(25, 15))
axs = axs.flatten()
labels = watermark_types
labels[1] = "xd23b"
for i, key in enumerate(wm_avg_var_dict_list[0].keys()):
    for k,watermark_type in enumerate(watermark_types):
        ...
        # 在子图中画出这个watermark_type的数据
        axs[i].bar(watermark_type, avg, yerr=var, label=watermark_type)
...
plt.savefig(f'./plot/clean_sentiment_new.pdf')