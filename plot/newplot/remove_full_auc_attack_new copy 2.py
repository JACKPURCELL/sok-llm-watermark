...
watermark_types = ["john23","xuandong23b","aiwei23","rohith23","aiwei23b","scott22"]
...
for i, watermark_type in enumerate(watermark_types):
    ...
    if "baseline_completion_stat_z_score" in clean_data_list[0]:
        ...
        clean_baseline_completion_prediction = [0 if not pred else 1 for pred in clean_baseline_completion_prediction]
        ...
        clean_w_wm_output_prediction = [0 if not pred else 1 for pred in clean_w_wm_output_prediction]
    ...
    y_pred = clean_baseline_completion_prediction + clean_w_wm_output_prediction
# 计算 ROC 曲线
    ...
    for j, attack in enumerate(attacks):
        ...
        if "baseline_completion_stat_z_score" in data_list[0]:
            ...
            baseline_completion_prediction = [0 if not pred else 1 for pred in baseline_completion_prediction]
            ...
            w_wm_output_attacked_prediction = [0 if not pred else 1 for pred in w_wm_output_attacked_prediction]
...
with open(f'./plot/newplot/output/output_{dataset}_{model}——auc.txt', 'w') as f:
