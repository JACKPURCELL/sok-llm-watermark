...
def main(args):
    ...
    lex_len = '40'
    model_path = "/home/jkl6486/sok-llm-watermark/runs/token_200/" + args.method + "/c4/opt/dipper_l" + lex_len + "_o0/dipp_roberta_finetuned_chatgpt_new"
    ...
    threshold = thresholds[args.method]

    count = 0

    data_path = f"/home/jkl6486/sok-llm-watermark/runs/token_200/{args.method}/c4/opt/gen_table_w_metrics.jsonl"
    with open(data_path, "r") as f:
        for line in f:
            ...
            labels.append(1)
            ### Other variables
            original_detector_predictions.append(1 if data["w_wm_output_z_score"] > threshold else 0)
    ...
    class dataset(Dataset):
        ...
        def __init__(self, generations, labels):
            ...
            self.labels = labels
            self.input_ids = tokenizer(generations, truncation=True, padding=True)['input_ids']
    ...
    # Compute idf dictionary for BERTScore TODO

    idf_dict = get_idf_dict(train_dataset.generations, tokenizer, nthreads=20)
    ...
    after_adv_change = []
    all_adv_texts = []
    assert args.start_index < len(train_dataset.generations), 'Starting index %d is larger than dataset length %d' % (args.start_index, len(train_dataset.generations))
    end_index = min(args.start_index + args.num_samples, len(train_dataset.generations))
    ...
    for idx in range(args.start_index, end_index):
        input_ids = train_dataset.input_ids[idx]
        ...
        if args.dataset == 'mnli':
            ...
            premise_length = len(tokenizer.encode(train_dataset.generations['premise'][idx]))
        ...
        with torch.no_grad():
            for j in range(args.gumbel_samples):
                ...
                if adv_logit.argmax() != label or j == args.gumbel_samples - 1:
                    ...
                    break
        all_adv_texts.append(adv_text)
        adv_output_labels.append(adv_logit.argmax().item())
        ori_output_labels.append(clean_logit.argmax().item())
        after_adv_change.append(adv_logit.argmax().item()!=label)        
    ...
    output_path = f"/home/jkl6486/sok-llm-watermark/runs/token_200/{args.method}/c4/opt/adv_perturb"
    os.makedirs(output_path, exist_ok=True)
    
    output_data_path = os.path.join(output_path,'gen_table_w_metrics.jsonl')
    
    source_path = f"/home/ljc/sok-llm-watermark/runs/token_200/{args.method}/c4/opt/dipper_l40_o0/gen_table_attacked_meta.json"
    shutil.copy(source_path, output_path)


    write_num=0
    with open(output_data_path, "w") as f ,  open(data_path, "r") as f2:
        for line in f2:
            if write_num >= args.num_samples:
                break
            data = json.loads(line)
            data["w_wm_output_attacked"] = all_adv_texts[write_num]
            data["w_wm_output_attacked_after_change"] = after_adv_change[write_num]
            json.dump(data, f)
            f.write('\n')
            write_num += 1
    ...
    torch.save({
        ...
        'entropies': entropies,
        'labels': [0 for _ in range(args.start_index,end_index)], 
...
if __name__ == "__main__":
    ...
    # Data 
    parser.add_argument("--data_folder", default='/home/jkl6486/text-adversarial-attack/', type=str,
    ...
    parser.add_argument("--model", default="gpt2", type=str,
        help="type of model")
    parser.add_argument("--method", default="xuandong23b", type=str,
        help="type of model")
